<?php
$id = (int)($_GET['id'] ?? 0);
$stmt = db_prepare($koneksi, "SELECT * FROM absensi WHERE id = ?");
db_stmt_bind_param($stmt, 'i', $id);
db_stmt_execute($stmt);
$abs = db_fetch_assoc(db_stmt_get_result($stmt));

if (!$abs) { header('Location: layout.php?page=absensi'); exit; }
...
$jadwalList = db_query($koneksi, "
    SELECT j.id, CONCAT(c.judul, ' - ', j.hari, ' ', TIME_FORMAT(j.jam_mulai,'%H:%i'), '-', TIME_FORMAT(j.jam_selesai,'%H:%i')) AS label
    FROM jadwal j JOIN courses c ON c.id = j.course_id ORDER BY j.hari, j.jam_mulai
");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($userId && $courseId && $tanggal) {
        $stmt2 = db_prepare($koneksi, "UPDATE absensi SET user_id=?, course_id=?, jadwal_id=?, tanggal=?, status=?, keterangan=? WHERE id=?");
        db_stmt_bind_param($stmt2, 'iiisssi', $userId, $courseId, $jadwalId, $tanggal, $status, $ket, $id);
        db_stmt_execute($stmt2);
        header('Location: layout.php?page=absensi&sukses=1');
        exit;
    }
    $error = 'Mahasiswa, course, dan tanggal wajib diisi.';
    $abs = array_merge($abs, $_POST);
}
?>
...
<div class="card shadow mb-4">
    <div class="card-body">
        ...
        <form method="post">
            <div class="form-group">
                ...
                <select name="user_id" class="form-control" required>
                    <?php while ($m = db_fetch_assoc($mahasiswaList)): ?>
                        <option value="<?php echo e($m['id']); ?>" <?php echo $abs['user_id'] == $m['id'] ? 'selected' : ''; ?>>
            ...
            <div class="form-group">
                ...
                <select name="course_id" class="form-control" required>
                    <?php while ($c = db_fetch_assoc($courses)): ?>
                        <option value="<?php echo e($c['id']); ?>" <?php echo $abs['course_id'] == $c['id'] ? 'selected' : ''; ?>>
                            <?php echo e($c['judul']); ?>
                        </option>
                    <?php endwhile; ?>
                </select>
            ...
            <div class="form-group">
                ...
                <select name="jadwal_id" class="form-control">
                    ...
                    <?php while ($j = db_fetch_assoc($jadwalList)): ?>
                        <option value="<?php echo e($j['id']); ?>" <?php echo $abs['jadwal_id'] == $j['id'] ? 'selected' : ''; ?>>
                            <?php echo e($j['label']); ?>
                        </option>
                    <?php endwhile; ?>
                </select>
            ...
            <div class="form-group">
                <label>Tanggal</label>
                <input type="date" name="tanggal" class="form-control" required value="<?php echo e($abs['tanggal']); ?>">
            </div>
            <div class="form-group">
                <label>Status</label>
                <select name="status" class="form-control" required>
                    <?php foreach (['hadir','izin','sakit','alpha'] as $s): ?>
                        <option value="<?php echo $s; ?>" <?php echo $abs['status'] === $s ? 'selected' : ''; ?>>
                            <?php echo ucfirst($s); ?>
                        </option>
                    <?php endforeach; ?>
                </select>
            ...
            <div class="form-group">
                ...
                <input type="text" name="keterangan" class="form-control" value="<?php echo e($abs['keterangan'] ?? ''); ?>">
            </div>
