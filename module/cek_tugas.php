...
$stmt = mysqli_prepare($koneksi, "SELECT s.*, u.nama_lengkap, u.email FROM assignment_submissions s JOIN users u ON u.id = s.user_id WHERE s.assignment_id = ? ORDER BY s.submitted_at DESC");
mysqli_stmt_bind_param($stmt, "i", $assignmentId);
mysqli_stmt_execute($stmt);
$submissions = mysqli_stmt_get_result($stmt);

$stmt = mysqli_prepare($koneksi, "SELECT u.nama_lengkap, u.email FROM enrollments e JOIN users u ON u.id = e.user_id LEFT JOIN assignment_submissions s ON s.assignment_id = ? AND s.user_id = u.id WHERE e.course_id = ? AND s.id IS NULL ORDER BY u.nama_lengkap");
mysqli_stmt_bind_param($stmt, "ii", $assignmentId, $assignment['course_id']);
mysqli_stmt_execute($stmt);
...
<div class="card shadow mb-4">
    ...
    <div class="card-body">
        <?php if (mysqli_num_rows($missing) === 0): ?>
            <p class="mb-0">Semua mahasiswa enroll sudah mengirim tugas.</p>
        <?php else: ?>
            <ul class="mb-0">
                <?php while ($student = mysqli_fetch_assoc($missing)): ?>
